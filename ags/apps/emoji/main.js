import { RegularWindow } from "../window.js";
import { MaterialIcon } from '../../modules/.commonwidgets/materialicon.js';
...
const RECENT_EMOJI_FILE = Gio.File.new_for_path(GLib.build_filenamev([GLib.get_home_dir(), ".cache", "recent_emoji.json"]));
...
function writeRecentEmoji(recent) {
    const data = JSON.stringify(recent, null, 2);
    RECENT_EMOJI_FILE.replace_contents(encoder.encode(data), null, false, Gio.FileCreateFlags.REPLACE_DESTINATION, null);
}
...
function EmojiList() {
    ...
    const Button = (icon, name) => Widget.Button({
