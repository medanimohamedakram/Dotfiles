import { current_wallpaper } from "../../variables.js";
...
// @ts-ignore
const focused = Variable(null);
function splitListInHalf(arr) {
    const midPoint = Math.ceil(arr.length / 2);
    const firstPart = arr.slice(0, midPoint);
    const secondPart = arr.slice(midPoint);
    return [firstPart, secondPart];
}
function listFilesByExtensions(directoryPath, extensions) {
    let files = [];
...
const Wallpaper = (image, name) => Widget.Button({
    on_clicked: () => {
            ...
            Widget.Box({
                ...
                setup(self) {
                    ...
                    Utils.readFileAsync(cache_file).catch(() => {
                        Utils.execAsync(`magick ${image} -resize x32 -gravity Center -extent 1:1 ${cache_file}`).catch(print);
                    });
                    Utils.idle(() => {
