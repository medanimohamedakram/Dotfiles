...
const WifiNetwork = (access_point) => {
    ...
    return Widget.Button({
        ...
        on_primary_click_release: () => {
            if (!connected)
                Utils.execAsync(`nmcli device wifi connect '${access_point.ssid}'`).catch(print);
            else if (is_saved)
...
function WifiList() {
    return Widget.Box({
        ...
        attribute: {
            updateNetworks: (self) => {
                var _a, _b;
                const accessPoints = ((_a = network.wifi) === null || _a === void 0 ? void 0 : _a.access_points) || [];
                const current_ssid = (_b = network.wifi) === null || _b === void 0 ? void 0 : _b.ssid;
                self.children = Object.values(accessPoints.reduce((a, accessPoint) => {
                    ...
                    // @ts-expect-error
                    .sort((a, b) => {
                    if (a.ssid === current_ssid)
                        return -1;
                    if (b.ssid === current_ssid)
                        return 1;
                    return 0;
                })
                    // @ts-expect-error
