...
export async function OpenSettings(cur_tab = "network") {
    if (current_window) {
...
function Settings(cur_tab) {
    ...
    const Row = (name, label, icon = "image-missing") => Widget.Button({
        on_clicked: () => {
...
export const Page = (widget, name) => Widget.Box({
    vertical: true,
    children: [PageTitle(name), widget]
});
export const PageTitle = (label) => Widget.Box({
...
export const SettingsWindow = (cur_tab) => {
